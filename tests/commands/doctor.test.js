...
describe('doctorCommand', () => {
  ...
  test('accepts deterministic documents generated by init --skip-ai', async () => {
    fs.mkdirSync(path.join(testDir, 'src'), { recursive: true });
    fs.writeFileSync(
      path.join(testDir, 'package.json'),
      JSON.stringify({ name: 'doctor-test', dependencies: { express: '^5.0.0' } })
    );
    fs.writeFileSync(path.join(testDir, 'src', 'index.js'), 'module.exports = { ready: true };');
    _clearCache();
    ...
    expect(report.quality.overall).toBe('OK');
  });
