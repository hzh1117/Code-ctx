...
describe('AI setup service', () => {
  ...
  test('writes provider config, protected env and gitignore then tests connection', async () => {
    ...
    const result = await setupAIConfig(
      rootDir,
      {
        provider: 'openai',
    ...
    expect(loadProjectConfig(rootDir).ai.openai).toEqual(
      expect.objectContaining({
  ...
  test('returns a structured connection failure without losing saved config', async () => {
    ...
    expect(fs.readFileSync(path.join(rootDir, '.env'), 'utf8')).toContain('ANTHROPIC_API_KEY=test-secret-key');
  });
});
