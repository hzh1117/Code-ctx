...
const hook = new Command('hook')
  ...
  .addCommand(
    new Command('install').description('安装 post-commit hook（commit 后提示更新文档）').action(async () => {
      try {
  ...
  .addCommand(
    new Command('uninstall').description('卸载 post-commit hook').action(async () => {
      try {
  ...
  .addCommand(
    new Command('status').description('查看 hook 安装状态').action(async () => {
      try {
