    ...
    runWithInterrupt(async signal => {
      try {
        ...
        if (!options.scenario && !result.lowConfidenceScenarios) {
          ...
          console.log(
            `✓ 识别为：场景 ${result.matchedScenario}（${result.scenarioName}）置信度 ${result.confidence}%${methodTag}`
          );
          if (result.aiReason) {
        ...
        const finalPrompt = options.nonInteractive ? result.prompt : await fillPlaceholders(result.prompt);
